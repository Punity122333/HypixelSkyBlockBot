...
class GatheringCommands(commands.Cog):
    ...
    async def mine(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "⛏️ **Choose your mining location:**", view=view, ephemeral=True
        )
    ...
    async def farm(self, interaction: discord.Interaction):
        ...
        if not progression or not progression.get("first_farm_date"):
            await self.bot.db.update_progression(
                interaction.user.id, first_farm_date=int(time.time())
            )
            await AchievementSystem.unlock_single_achievement(
                self.bot.db, interaction, interaction.user.id, "first_farm"
            )

        active_events = await self.event_effects.get_active_events()
        event_multiplier = (
            await self.event_effects.get_gathering_multiplier()
            if active_events
            else 1.0
        )
        xp_multiplier = (
            await self.event_effects.get_xp_multiplier("farming")
            if active_events
            else 1.0
        )
        coin_multiplier = (
            await self.event_effects.get_coin_multiplier() if active_events else 1.0
        )
        fortune_bonus = (
            await self.event_effects.get_fortune_bonus("farming")
            if active_events
            else 0
        )
        ...
        for crop_type in selected_crops:
            result = await GatheringSystem.harvest_crop(
                self.bot.db, interaction.user.id, crop_type, event_bonuses
            )
            ...
            if result["success"]:
                ...
                for drop in result["drops"]:
                    ...
                    await self.bot.db.update_collection(
                        interaction.user.id, item_id, amount
                    )
        ...
        if farming_skill:
            ...
            new_level = await self.bot.game_data.calculate_level_from_xp(
                "farming", new_xp
            )
            await self.bot.db.update_skill(
                interaction.user.id, "farming", xp=new_xp, level=new_level
            )

        await AchievementSystem.check_skill_achievements(
            self.bot.db, interaction, interaction.user.id, "farming", new_level
        )

        from utils.systems.badge_system import BadgeSystem

        await BadgeSystem.check_and_unlock_badges(
            self.bot.db,
            interaction.user.id,
            "skill",
            skill_name="farming",
        ...
        if new_level >= 50:
            await BadgeSystem.check_and_unlock_badges(
                self.bot.db, interaction.user.id, "skill_50"
            )
        ...
        if tool_bonuses_display:
            ...
            if tool_bonuses_display.get("fortune", 0) > 0:
                tool_bonus_text.append(
                    f"+{int(tool_bonuses_display['fortune'])} Farming Fortune"
                )
            if tool_bonuses_display.get("yield_multiplier", 1.0) > 1.0:
                tool_bonus_text.append(
                    f"{tool_bonuses_display['yield_multiplier']:.2f}x Crop Yield"
                )
            if tool_bonus_text:
        ...
        active_events = await self.event_effects.get_active_events()
        if active_events and (
            event_multiplier > 1.0
            or xp_multiplier > 1.0
            or coin_multiplier > 1.0
            or fortune_bonus > 0
        ):
        ...
        if len(items_found.items()) != 0:
            ...
            embed.add_field(
                name="No Crops Found", value="Better luck next time!", inline=False
            )
        embed.add_field(
            name="💰 Coins Earned", value=f"+{total_coins} coins", inline=False
        )
        embed.add_field(name="⭐ Farming XP", value=f"+{total_xp} XP", inline=False)
        embed.add_field(
            name="Current Level", value=f"Farming {new_level}", inline=False
        )
        await interaction.followup.send(embed=embed)
    ...
    async def fish(self, interaction: discord.Interaction):
        ...
        if not progression or not progression.get("first_fish_date"):
            await self.bot.db.update_progression(
                interaction.user.id, first_fish_date=int(time.time())
            )
            await AchievementSystem.unlock_single_achievement(
                self.bot.db, interaction, interaction.user.id, "first_fish"
            )

        active_events = await self.event_effects.get_active_events()
        xp_multiplier = (
            await self.event_effects.get_xp_multiplier("fishing")
            if active_events
            else 1.0
        )
        coin_multiplier = (
        ...
        sea_creature_bonus = (
            await self.event_effects.get_sea_creature_bonus() if active_events else 0
        )
        ...
        result = await GatheringSystem.fish(
            self.bot.db, interaction.user.id, event_bonuses
        )

        if not result["success"]:
            await interaction.followup.send(
                f"❌ {result.get('error', 'Failed to fish')}", ephemeral=True
            )
            return
        ...
        if is_sea_creature:
            ...
            embed.add_field(
                name="Enemy Health", value=f"❤️ {creature_health} HP", inline=True
            )
            embed.add_field(
                name="Enemy Damage", value=f"⚔️ ~{creature_damage} damage", inline=True
            )
            embed.add_field(
                name="Potential Reward",
                value=f"💰 {catch['coins']} coins\n⭐ {catch['xp']} XP",
                inline=False,
            )
            ...
            await self.bot.db.add_item_to_inventory(
                interaction.user.id, catch["item_id"], catch["amount"]
            )
            await self.bot.db.update_collection(
                interaction.user.id, catch["item_id"], catch["amount"]
            )
            ...
            if tool_bonuses.get("fortune", 0) > 0:
                tool_bonus_text.append(
                    f"+{int(tool_bonuses['fortune'])}% Sea Creature Chance"
                )
            if tool_bonus_text:
                current_desc = embed.description or ""
                embed.description = f"{current_desc}\n🔧 **Tool Bonuses:** {' • '.join(tool_bonus_text)}"
            ...
            embed.add_field(
                name=item_name,
                value=f"{catch['amount']}x ({catch['rarity'].upper()})",
                inline=False,
            )
        skills = await self.bot.db.get_skills(interaction.user.id)
        ...
        if fishing_skill:
            ...
            await self.bot.db.update_skill(
                interaction.user.id, "fishing", xp=new_xp, level=new_level
            )

        await AchievementSystem.check_skill_achievements(
            self.bot.db, interaction, interaction.user.id, "fishing", new_level
        )
        ...
        active_events = await self.event_effects.get_active_events()
        if active_events and (
            xp_multiplier > 1.0 or coin_multiplier > 1.0 or sea_creature_bonus > 0
        ):
            event_text = "🎪 **Active Event Bonuses:** "
            bonuses = []
            if sea_creature_bonus > 0:
        ...
        embed.add_field(
            name="Current Level", value=f"Fishing {new_level}", inline=False
        )
        await interaction.followup.send(embed=embed)
    ...
    async def forage(self, interaction: discord.Interaction):
        ...
        if not progression or not progression.get("first_forage_date"):
            await self.bot.db.update_progression(
                interaction.user.id, first_forage_date=int(time.time())
            )
            await AchievementSystem.unlock_single_achievement(
                self.bot.db, interaction, interaction.user.id, "first_forage"
            )
        ...
        xp_multiplier = (
            await self.event_effects.get_xp_multiplier("foraging")
            if active_events
            else 1.0
        )
        coin_multiplier = (
        ...
        fortune_bonus = (
            await self.event_effects.get_fortune_bonus("foraging")
            if active_events
            else 0
        )
        ...
        for wood_type in selected_woods:
            result = await GatheringSystem.chop_tree(
                self.bot.db, interaction.user.id, wood_type, event_bonuses
            )
        ...
        skills = await self.bot.db.get_skills(interaction.user.id)
        foraging_skill = next(
            (s for s in skills if s["skill_name"] == "foraging"), None
        )
        ...
        if foraging_skill:
            ...
            await self.bot.db.update_skill(
                interaction.user.id, "foraging", xp=new_xp, level=new_level
            )

        await AchievementSystem.check_skill_achievements(
            self.bot.db, interaction, interaction.user.id, "foraging", new_level
        )
        ...
        if tool_bonuses_display:
            ...
            if tool_bonuses_display.get("fortune", 0) > 0:
                tool_bonus_text.append(
                    f"+{int(tool_bonuses_display['fortune'])} Foraging Fortune"
                )
            if tool_bonuses_display.get("yield_multiplier", 1.0) > 1.0:
                tool_bonus_text.append(
                    f"{tool_bonuses_display['yield_multiplier']:.2f}x Wood Yield"
                )
            if tool_bonus_text:
                current_desc = embed.description or ""
                embed.description = f"{current_desc}\n🔧 **Tool Bonuses:** {' • '.join(tool_bonus_text)}"
        ...
        if len(items_found.items()) != 0:
            ...
            embed.add_field(
                name="No Wood Found", value="Better luck next time!", inline=False
            )
        embed.add_field(
            name="💰 Coins Earned", value=f"+{total_coins} coins", inline=False
        )
        ...
        embed.add_field(
            name="Current Level", value=f"Foraging {new_level}", inline=False
        )
        await interaction.followup.send(embed=embed)
    ...
    async def taming(self, interaction: discord.Interaction):
        ...
        player_stats = await StatCalculator.calculate_player_stats(
            self.bot.db, self.bot.game_data, interaction.user.id
        )
        ...
        if taming_skill:
            ...
            await self.bot.db.update_skill(
                interaction.user.id, "taming", xp=new_xp, level=new_level
            )

        await AchievementSystem.check_skill_achievements(
            self.bot.db, interaction, interaction.user.id, "taming", new_level
        )
        ...
        if xp_multiplier > 1.0:
            event_text = (
                f"🎪 **Active Event Bonuses:** +{int((xp_multiplier - 1) * 100)}% XP"
            )
            current_desc = embed.description or ""
            embed.description = f"{current_desc}\n{event_text}"
        embed.add_field(name="⭐ Taming XP", value=f"+{xp_gained} XP", inline=False)
        embed.add_field(
            name="🍀 Pet Luck Bonus", value=f"{pet_luck:.1f}%", inline=False
        )
        await interaction.followup.send(embed=embed)
