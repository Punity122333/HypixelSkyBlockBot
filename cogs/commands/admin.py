...
class AdminCommands(commands.Cog):
    ...
    async def cog_autocomplete(self, interaction: discord.Interaction, current: str):
        ...
        filtered_cogs = [
            app_commands.Choice(
                name=cog.replace("cogs.", "").replace(".", " > "), value=cog
            )
            for cog in cogs
    ...
    @app_commands.command(
        name="sync_commands", description="Force sync commands to Discord"
    )
    ...
    async def sync_commands(
        ...
        try:
            if interaction.guild:
                ...
                synced_count = await self.bot.sync_manager.safe_sync(
                    guild=guild, force=force
                )
                location = f"guild {interaction.guild.name}"
            else:
                synced_count = await self.bot.sync_manager.safe_sync(
                    guild=None, force=force
                )
                location = "globally"
    ...
    @app_commands.command(
        name="clear_sync_cache", description="Clear command sync cache"
    )
