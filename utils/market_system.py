...
class MarketSystem:
    ...
    async def update_bazaar_prices(self):
        if not self.game_data:
            return
        all_items = await self.game_data.get_all_items()
    ...
    async def create_buy_order(self, user_id: int, product_id: str, price: float, amount: int) -> tuple[bool, str]:
        ...
        await self.db.create_bazaar_order(user_id, product_id, 'BUY', price, amount)
    ...
    async def create_sell_order(self, user_id: int, product_id: str, price: float, amount: int) -> tuple[bool, str]:
        ...
        await self.db.remove_item_from_inventory(user_id, product_id, amount)
        await self.db.create_bazaar_order(user_id, product_id, 'SELL', price, amount)
        ...
        for auction in auctions:
            ...
            if auction['bin'] and auction['buy_now_price'] < base_value * (1 - self.risk_tolerance * 0.3):
                if self.coins >= auction['buy_now_price']:
                    success = await self.db.buy_bin(auction['id'], -self.bot_id)
                    if success:
...
async def simulate_stock_market(db: GameDatabase):
    ...
    for stock in stocks:
        ...
        await db.update_stock_price(stock['stock_symbol'], new_price, volume)

async def generate_merchant_deals(db: GameDatabase, game_data):
    ...
    if len(deals) < 3:
        ...
        if item:
            base_price = item.default_bazaar_price
            ...
            await db.create_merchant_deal(merchant, item_id, quantity, price, deal_type, duration)
...
async def run_market_simulation(db: GameDatabase, market: MarketSystem):
    try:
        ...
        bazaar_traders = [BazaarBotTrader(db, market, bot) for bot in bazaar_bots]
        
        for trader in bazaar_traders:
            await trader.execute_trading_strategy()
        
        auction_bots = await db.get_auction_bots()
        auction_traders = [AuctionBotTrader(db, market, bot) for bot in auction_bots]
        ...
        if random.random() > 0.7 and market.game_data:
